...
"""
    ...
    """
    Generate a comprehensive crop analysis including market insights.
    
    Parameters:
    -----------
    crop : str
        Crop name for analysis
    
    Returns:
    --------
    Optional[str] : Analysis text or None on error
    """
    
    try:
        prompt = f"""Provide a brief analysis for {crop}:
1. Market demand
2. Typical yield range
3. Common challenges
...
def generate_disease_prevention_advice(crop: str, season: str) -> Optional[str]:
    """
    Generate disease prevention and management advice for a specific crop.
    ...
    Returns:
    --------
    Optional[str] : Disease prevention advice or None on error
    """
    
    try:
        prompt = f"""Provide disease prevention advice for {crop} during {season} season:
1. Common diseases/pests
